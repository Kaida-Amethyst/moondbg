...
`acceptance/dap_build_wbtest.mbt` 使用临时项目验证按包构建、修改后再次 F5 的新值、
编译失败不使用旧二进制，以及 `/tmp` 符号链接路径的源码断点。它还用 cc 编译一个
会派生子进程的慢速假 moon，在隔离的临时 MOON_HOME 下验证 disconnect 和 EOF 都会
终止 worker、moon 及其子进程；不修改用户的工具链或软链接。默认测试覆盖准备阶段事件、
失败/取消时序及拆分 UTF-8 构建输出，CLI 的原有包选择测试已迁入 `launch/`。
...
- 完整真实验收：20/20 通过，启动当时构建的 `repl.exe`（后续入口已重命名为 `main.exe`）。
