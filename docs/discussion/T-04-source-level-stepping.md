...
三个命令的固定缩写分别是 `n`、`s` 和 `fin`。这项决定在 P3 首次完成后追加，取代此前
“只接受完整命令名”的限制；除这三个固定缩写外，其他缩写、同义命令和带参数形式仍不在
本轮范围内。三个命令都只允许在 `Stopped` 状态执行，并使用最近一次 stop event 所确定的
当前线程。首版不增加线程切换或栈帧选择功能。
...
1. 在命令模型和解析器中加入三个无参数命令及本主题确认的固定缩写；
...
`StepOut` 三种内部语义，接受无参数的完整命令名 `next`、`step`、`finish` 以及固定缩写
`n`、`s`、`fin`；其他缩写和近似前缀保持为未知命令，额外参数会在本地解析阶段报错。
`help` 已补充三个命令的越过、进入和跳出说明及其缩写。
...
命令单元测试覆盖三个完整名称、三个固定缩写、前后空白、额外参数和近似前缀。新增独立
fake REPL 流程，先在 `Ready` 状态验证三个动作分别被拒绝，再运行到停点并通过缩写连续
执行 `n → s → fin`；测试核对三次 `Stopped (step)` 反馈、每次 prompt 恢复，以及 trace 中
恰好各有一个 `next`、`stepIn` 和 `stepOut` request。最终 `moon check` 无警告，76 个
