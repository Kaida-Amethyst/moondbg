...
支持的语法为变量名后接 `.字段`、`.N` 位置成员和 `[非负整数字面量]`，例如
`lines[0].start.x`、`matrix[1][2]`；允许首尾空白，不允许路径内部空白。
暂不支持 `arr[i]`、包级变量、算术、赋值、函数调用或 REPL 命令。
tuple / 当前 enum 构造器的载荷支持 `pair.0`、`result.0.x`、`list.1.0`，
展开后的成员提供对应 Add to Watch 表达式。位置成员内的复合数组读取仍受 LLDB 限制，
例如 `values.nested.1[0]` 会明确报错（详见 README 的 tuple / Result 小节）。
